...
<summary>This section provides step-by-step instructions for setting up the working environment, running the main script, and navigating the project’s interactive menu to process and validate datasets.</summary>
    1. Data used should be generated by `Import and format YYYY data.R`
    2. Place your data directories into the `data` directory.
    3. Run `Rscript script.r` to process the data. This will generate files in the `/repack` directory and output files in the `/out` directory.
    4. Follow the prompted options to validate or generate files as required.
